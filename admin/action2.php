...
$path = rtrim('../i/img', DIRECTORY_SEPARATOR) . '/' . $fullName;

$crop = new App_Util_Crop();
$crop->initialize($src, $path, $_GET['x'], $_GET['y'], 800, 600, $_GET['w'], $_GET['h']);
$success = $crop->generate_shot();
//print_r($success);
$msg = $success ? '裁剪成功' : '裁剪失败';
echo json_encode(array('result' => $success, 'msg' => $msg, 'src' => 'https://hola.party/i/img/'.$fullName));
//print_r($success);